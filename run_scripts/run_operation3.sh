...
$library work
$compile -writetoplevels questa.tops -timescale 1ps/1fs -f run_operation3.f 
$sim -f questa.tops -c -do "vsim -voptargs=+acc=npr; run -all; exit" -voptargs=+acc=npr | tee $LOG/log_operation3.txt
